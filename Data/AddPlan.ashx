...
public class AddPlan : IHttpHandler {
    ...
    public void errorApply(HttpContext context) 
    {
        try
        {
            ...
            num.PlanNum = Convert.ToInt32(context.Request["PlanNum"]);//计划
            num.Code = context.Request["label"].ToString();//部品
            ...
            string sql = "insert into ProducePlan(jz,ItemCode,PlanNum,LabelCode,users,times,Remarks,SingleId) values('" + num.jz + "','" + num.ItemCode + "','" + num.PlanNum + "','"+num.Code+"','" + num.users + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "','异常申请','" + avgId[0] + "')";
    ...
    public void Apply(HttpContext context)
    {
        try
        {
            ...
            string sql = "insert into ProducePlan(jz,ItemCode,PlanNum,LabelCode,users,times,Remarks,SingleId) values('" + num.jz + "','" + num.ItemCode + "','" + num.PlanNum + "','ALL','" + num.users + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "','正常申请','" + avgId[0] + "')";
            db.ExcuteSqlScalar("SqlServer", sql);
            ht.Add("state", "0");
            result.Add(ht);
            context.Response.Write(JsonConvert.SerializeObject(result));
